...
function Principal() {
  ...
  return (
    <div className="flex min-h-screen bg-[#F0F5FA]">
      {/* LEFT SIDEBAR - Fixed */}
      <aside className="w-90 bg-indigo-100 p-6 hidden md:flex flex-col shadow-lg fixed h-full border-r border-indigo-200">
        <div className="flex items-center gap-3 mb-10">
          <div className="w-12 h-12 bg-white rounded-full border-2 border-indigo-500 flex items-center justify-center font-bold text-indigo-700">
          ...
          <h2 className="text-2xl font-extrabold tracking-wider text-transparent bg-clip-text bg-gradient-to-r from-gray-800 to-indigo-600 drop-shadow-sm">
            PRINCIPAL
          </h2>
        </div>
...
const NavItem = ({ icon, label, active, onClick }) => (
  <div
    onClick={onClick}
    className={`flex items-center gap-4 p-4 rounded-2xl cursor-pointer transition-all ${active ? 'bg-indigo-600 text-white shadow-md shadow-indigo-300' : 'text-slate-700 hover:bg-indigo-200 hover:text-indigo-900'
      }`}
  >
